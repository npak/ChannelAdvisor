...
{
    public class MorrisChangesService : IVendorService
    {
        ...
        /// 
        /// </summary>
        public MorrisChangesService()
        {
            new DAL().GetMorrisChangesSettings(out _url, out _dropshipfree, out csvfolder, out csvfilename, out csvIsftp);

            VendorInfo = new DAL().GetVendor((int)VendorName.MorrisChanges);

            if (VendorInfo == null)
                throw new Exception(string.Format("Cannot load data for Morris Changes"));
        }//end constructor
        ...
        private InventoryUpdateServiceDTO CreateInvUpdateServiceDTOForMorris(bool isWinForm)
        {
            ...
            BindingList<Inventory> lstMorrisChangesInventory = new BindingList<Inventory>();
            XmlNodeList nodes;
            Inventory invDTO;
            ...
            for (int i = 1; i < XMLFileCount; i++)
            {
                ...
                if (!isWinForm)
                {
                    ...
                    string vendorFile = CAUtil.SaveXMLAsVendorFile(xmlInventory, (int)VendorName.MorrisChanges);
                ...
                foreach (XmlNode node in nodes)
                {
                    ...
                    lstMorrisChangesInventory.Add(invDTO);
                }//end for each
            }
