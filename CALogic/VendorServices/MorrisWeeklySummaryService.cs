...
{
    public class MorrisWeeklySummaryService : IVendorService
    {
        ...
        /// 
        /// </summary>
        public MorrisWeeklySummaryService()
        {
            new DAL().GetMorrisWeeklySummarySettings(out _url,out csvfolder, out csvfilename, out creditcsvfolder, out creditcsvfilename);
            _url = GetCurrentUrl();
            VendorInfo = new DAL().GetVendor((int)VendorName.MorrisWeeklySummary);
                    ...
                    foreach (XmlNode node in nodes)
                    {
                           ...
                           (node["otype"].InnerXml.Trim() == "Return" && node["Status"].InnerXml.Trim() == "Invoiced"))
                        {
                            ...
                            else if (node["otype"].InnerXml.Trim() == "Return")
                            {
                                ...
                                returnBuilder.Append(TrackNum).Append(",");
                                returnBuilder.AppendLine();
                            }
        ...
        private string GetCurrentUrl()
        {
            ...
            {
                dt = dt.AddDays(-(Int32)dt.DayOfWeek); 
            }
