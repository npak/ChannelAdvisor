...
{
    public class MorrisCompleteService : IVendorService
    {
        ...
        /// 
        /// </summary>
        public MorrisCompleteService()
        {
            new DAL().GetMorrisCompleteSettings(out _url, out _dropshipfree, out csvfolder, out csvfilename, out csvIsftp);

            VendorInfo = new DAL().GetVendor((int)VendorName.MorrisComplete);

            if (VendorInfo == null)
                throw new Exception(string.Format("Cannot load data for Morris Complete"));
        }//end constructor
        ...
        {
            return CreateInvUpdateServiceDTOForMorrisComplete(false);
        }//end method
        ...
        public InventoryUpdateServiceDTO GetInventoryListForPreviewAndUpdate(int profileID)
        {
            ...
            InventoryUpdateServiceDTO invUpdateSrcvDTO = CreateInvUpdateServiceDTOForMorrisComplete(true);
        ...
        /// </summary>
        /// <returns></returns>
        private InventoryUpdateServiceDTO CreateInvUpdateServiceDTOForMorrisComplete(bool isWinForm)
        {
            //string UDT = "";
            string date = "";
            string type = "";
            ...
            for (int i = 1; i < XMLFileCount; i++)
            {
                ...
                if (!isWinForm)
                {
                    ...
                    string vendorFile = CAUtil.SaveXMLAsVendorFile(xmlInventory, (int)VendorName.Morris);
                ...
                foreach (XmlNode node in nodes)
                {
                    ...
                    if (detail["Average_Shipping_Cost"] == null)
                        invDTO.AvrShiftCost = null;
                    else
                    {
                        invDTO.AvrShiftCost = float.Parse(detail["Average_Shipping_Cost"].InnerText.Trim());
                        invDTO.AvrShiftCost = (float)(Math.Round((double)invDTO.AvrShiftCost, 2));
                    }

                    if (detail["MapPrice"] == null)
                        invDTO.MAP  = 0;
                    else
                    {
                        map = detail["MapPrice"].InnerText.Trim();
                        if (float.TryParse(map, out f))
                            invDTO.MAP = f;        //float.Parse(detail["MapPrice"].InnerText.Trim());
                        else 
                            invDTO.MAP = 0;
        ...
        {
            //test
            //string ss = _url.Replace("001", i.ToString("000"));
           // i = 999;
            try
            {
