...
{
    public class MorrisService : IVendorService
    {
        ...
        {
            new DAL().GetMorrisSettings(out _url, out _dropshipfree, out csvfolder, out csvfilename, out csvIsftp);
        ...
        /// <returns></returns>
        /// 
        private XmlDocument GetInventoryXMLOLD()
        {
            System.Net.ServicePointManager.ServerCertificateValidationCallback = ((sender, cert, chain, Errors) => true);

            PostSubmitter post = new PostSubmitter();
            post.Url = _url;
            post.PostItems.Add("fields", _fields);
            post.PostItems.Add("delim", _delim);
            post.PostItems.Add("id", _id);
            post.PostItems.Add("time", _time);
